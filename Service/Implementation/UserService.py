...
class UserService(IUserService, ABC):
    ...
    def generate_token(self, username: Union[str, Any]):
        try:
            ...
            payload = {
                "exp": datetime.utcnow() + timedelta(seconds=60 * 60 * 24 * 3), "username": username
            }
            encoded_jwt = jwt.encode(payload, SECRET_KEY, algorithm=SECURITY_ALGORITHM)
            return encoded_jwt
    ...
    def register_user(self, user_model: UserModel) -> [bool]:
        try:
            ...
            user_id = self.user_repository.max(Users.id)
            if user_id:
        ...
        try:
            ...
            if user_entity is not None and len(user_entity) > 0:
                if bcrypt.checkpw(bytes(user_model.password, 'utf-8'), user_entity[0].password):
                    return UserService.__generate_token(username=user_model.user_name)
                else:
